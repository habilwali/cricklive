import { Skeleton } from "../ui/skeleton";

const ScorecardSkeleton = () => {
    return (
        <div className=" ">
            <div className="grid lg:grid-cols-3 gap-2 mt-3">
            <Skeleton className="lg:w-[440.84px] md:w-[340.84px]   w-auto  h-[192px] relative rounded-[12.23px] bg-white" >
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 left-[50px] mt-2   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 bottom-0  left-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  ms-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>

                <div className="flex flex-col   justify-center align-middle items-center absolute top-0 bottom-0  right-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  me-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px]  mt-2" />
                      </div>
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute bottom-0 left-[50px] mb-3   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div>

              </Skeleton>
              
              <Skeleton className="lg:block hidden lg:w-[440.84px] md:w-[340.84px] lg:visible invisible w-[320.84px]  h-[192px] relative rounded-[12.23px] bg-white" >
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 left-[50px] mt-2   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 bottom-0  left-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  ms-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>

                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 bottom-0  right-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  me-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px]  mt-2" />
                      </div>
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute bottom-0 left-[50px] mb-3   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div>

              </Skeleton>
              <Skeleton className=" lg:block hidden lg:w-[440.84px] md:w-[340.84px] lg:visible invisible  w-[320.84px]  h-[192px] relative rounded-[12.23px] bg-white" >
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 left-[50px] mt-2   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 bottom-0  left-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  ms-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>

                <div className="flex flex-col  justify-center align-middle items-center absolute top-0 bottom-0  right-8">
                  <div className="flex justify-between ">
                    <div className="flex justify-between">
                      <div className="flex  mt-5 flex-col items-center  lg:ms-4  me-2 " >
                        <Skeleton className="h-4 lg:w-[70px] w-[50px]" />
                        <Skeleton className="h-4 w-[30px]  mt-2" />
                      </div>
                      <div className=" flex justify-center items-center   flex-col">
                        <Skeleton className=" w-[47.48px] h-[47.48px] rounded-full flex justify-center items-center" />
                        <Skeleton className="h-4 w-[50px] mt-2" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col  justify-center align-middle items-center absolute bottom-0 left-[50px] mb-3   ">
                  <Skeleton className="h-4 lg:w-[250px] w-[200px] mt-2" />
                </div> 

              </Skeleton>

            </div>
          </div>
      );
}
 
export default ScorecardSkeleton;